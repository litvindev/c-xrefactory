...
(defvar c-xref-java-suffixes '(".java"))
...
(defvar c-xref-ppc-progress (format "<%s>" PPC_PROGRESS))
...
(defvar c-xref-refactorings-assoc-list
  (list
   (list PPC_AVR_NO_REFACTORING "No refactoring" 'c-xref-no-refactoring nil)
   (list PPC_AVR_RENAME_SYMBOL "Rename Symbol" 'c-xref-rename-symbol nil)
   (list PPC_AVR_RENAME_CLASS "Rename Class" 'c-xref-rename-class nil)
   (list PPC_AVR_RENAME_PACKAGE "Rename Package" 'c-xref-rename-package nil)
   (list PPC_AVR_ADD_PARAMETER "Add Parameter" 'c-xref-add-parameter nil)
   (list PPC_AVR_DEL_PARAMETER "Delete Parameter" 'c-xref-del-parameter nil)
   (list PPC_AVR_MOVE_PARAMETER "Move Parameter" 'c-xref-move-parameter nil)
   (list PPC_AVR_EXTRACT_METHOD "Extract Method" 'c-xref-extract-method nil)
   (list PPC_AVR_EXTRACT_FUNCTION "Extract Function" 'c-xref-extract-function nil)
   (list PPC_AVR_EXTRACT_MACRO "Extract Macro" 'c-xref-extract-macro nil)
   (list PPC_AVR_MOVE_STATIC_FIELD "Move Static Field" 'c-xref-move-static-field nil)
   (list PPC_AVR_MOVE_STATIC_METHOD "Move Static Method" 'c-xref-move-static-method nil)
   (list PPC_AVR_MOVE_FIELD "Move Field" 'c-xref-move-field nil)
   (list PPC_AVR_PULL_UP_FIELD "Pull Up Field" 'c-xref-pull-up-field nil)
   (list PPC_AVR_PULL_UP_METHOD "Pull Up Method" 'c-xref-pull-up-method nil)
   (list PPC_AVR_PUSH_DOWN_FIELD "Push Down Field" 'c-xref-push-down-field nil)
   (list PPC_AVR_PUSH_DOWN_METHOD "Push Down Method" 'c-xref-push-down-method nil)
   (list PPC_AVR_MOVE_CLASS "Move Class" 'c-xref-move-class nil)
   (list PPC_AVR_MOVE_CLASS_TO_NEW_FILE "Move Class to New File" 'c-xref-move-class-to-new-file nil)
   (list PPC_AVR_MOVE_ALL_CLASSES_TO_NEW_FILE "Move file" 'c-xref-move-file nil)
   (list PPC_AVR_ENCAPSULATE_FIELD "Encapsulate Field" 'c-xref-encapsulate-field nil)
   (list PPC_AVR_SELF_ENCAPSULATE_FIELD "Self Encapsulate Field" 'c-xref-self-encapsulate-field nil)
   (list PPC_AVR_TURN_DYNAMIC_METHOD_TO_STATIC "Turn Virtual Method to Static" 'c-xref-turn-dynamic-method-to-static nil)
   (list PPC_AVR_TURN_STATIC_METHOD_TO_DYNAMIC "Turn Static Method to Virtual" 'c-xref-turn-static-method-to-dynamic nil)
   (list PPC_AVR_ADD_TO_IMPORT "Add import" 'c-xref-add-to-imports nil)
   (list PPC_AVR_REDUCE_NAMES "Reduce names" 'c-xref-reduce-names nil)
   (list PPC_AVR_EXPAND_NAMES "Expand names" 'c-xref-expand-names nil)
   (list PPC_AVR_ADD_ALL_POSSIBLE_IMPORTS "Add all used imports" 'c-xref-reduce-long-names-in-the-file nil)
   (list PPC_AVR_SET_MOVE_TARGET "Set Target for Next Moving Refactoring" 'c-xref-set-moving-target-position nil)
   (list PPC_AVR_UNDO "Undo Last Refactoring" 'c-xref-undo-last-refactoring nil)
))
...
(defun c-xref-server-tags-process (opts)
  (let ((bl) (enc))
	...
	(setq opts (append opts (list "-errors" 
								  "-xrefrc" c-xref-options-file
								  "-p" c-xref-active-project
								  "-user" (int-to-string (c-xref-get-this-frame-id))
								  )))
	(setq enc (c-xref-encoding-option))
	(if (not (equal enc ""))
		(setq opts (append (list enc) opts))
...
(defun c-xref-server-parse-xml-tag (ss i len)
  (let ((b) (e) (att) (attval))
	(setq c-xref-server-ctag PPC_NO_TAG)
	(setq i (c-xref-server-dispatch-skip-blank ss i len))
	(if (< i len)
		(progn
		  (if (not (eq (elt ss i) ?<))
...
(defun c-xref-server-dispatch-license-error (ss i len dispatch-data)
  (let ((tlen) (cc) (cw) (sel) (ln))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_LICENSE_ERROR)
...
(defun c-xref-server-dispatch-add-to-imports-dialog (ss i len dispatch-data)
  (let ((tlen) (cc) (cw) (dw) (cl) (pack) (copt) (default) (sel))
	(setq cw (selected-window))
	(setq default (c-xref-server-dispatch-get-int-attr PPCA_VALUE))
	(setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	(setq cc (c-xref-char-list-substring ss i (+ i tlen)))
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_ADD_TO_IMPORTS_DIALOG)
	(c-xref-select-dispach-data-caller-window dispatch-data)
...
(defun c-xref-server-dispatch-call-macro (ss i len dispatch-data)
  (let ((tlen) (cc))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_CALL_MACRO)
...
(defun c-xref-server-dispatch-kill-buffer-remove-file (ss i len dispatch-data)
  (let ((tlen) (cc))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_KILL_BUFFER_REMOVE_FILE)
...
(defun c-xref-server-dispatch-no-project (ss i len dispatch-data)
  (let ((tlen) (cc))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_NO_PROJECT)
...
(defun c-xref-server-dispatch-bottom-information (ss i len dispatch-data tag)
  (let ((tlen) (cc) (beep))
	(setq beep (c-xref-server-dispatch-get-int-attr PPCA_BEEP))
	(setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	(setq cc (c-xref-char-list-substring ss i (+ i tlen)))
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
...
(defun c-xref-server-dispatch-single-completion (ss i len dispatch-data)
  (let ((tlen) (cc))
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_SINGLE_COMPLETION)
	i
))
...
(defun c-xref-server-dispatch-set-info (ss i len dispatch-data)
  (let ((tlen) (cc))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_SET_INFO)
...
(defun c-xref-server-dispatch-browse-url (ss i len dispatch-data)
  (let ((tlen) (url))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_BROWSE_URL)
...
(defun c-xref-server-dispatch-fqt-completion (ss i len dispatch-data)
  (let ((tlen) (cc) (pack) (cl) (sel))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_FQT_COMPLETION)
	(setq cl (c-xref-file-name-extension cc))
	(setq pack (c-xref-file-name-without-suffix cc))
	(setq sel (c-xref-modal-dialog c-xref-selection-modal-buffer (format
...
(defun c-xref-server-dispatch-available-refactorings (ss i len dispatch-data)
  (let ((code) (selectedref) (srefn) (selectedline) (refs) (menu) 
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(while (equal c-xref-server-ctag PPC_INT_VALUE)	  
	  (setq code (c-xref-server-dispatch-get-int-attr PPCA_VALUE))
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	  ...
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  (c-xref-server-dispatch-require-end-ctag PPC_INT_VALUE)
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  )
	(c-xref-server-dispatch-require-end-ctag PPC_AVAILABLE_REFACTORINGS)
	(setq menu (format "%s\n---\n" menu))
...
(defun c-xref-server-dispatch-all-completions (ss i len dispatch-data)
  (let ((tlen) (cc) (nofocus))
	...
	(insert-string cc)
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_ALL_COMPLETIONS)
	(c-xref-line-hightlight 0 (point-max) nil 1 c-xref-font-lock-compl-keywords t)
	(goto-char (point-min))
	(c-xref-server-dispatch-show-completions-buffer (eq nofocus 0) dispatch-data)
	i
...
(defun c-xref-server-dispatch-goto (ss i len dispatch-data)
  (let ((tlen) (path) (offset) (line) (col) (rpos))
	...
	(setq i (+ i tlen))
	(if (equal c-xref-server-ctag PPC_OFFSET_POSITION)
		(progn
		  (setq offset (c-xref-server-dispatch-get-int-attr PPCA_OFFSET))
		  (c-xref-select-dispach-data-caller-window dispatch-data)
		  (c-xref-find-file path)
		  (goto-char (+ offset 1))
		  (setq i (c-xref-server-parse-xml-tag ss i len))
		  (c-xref-server-dispatch-require-end-ctag PPC_OFFSET_POSITION)
		  )
	  (c-xref-server-dispatch-require-ctag PPC_LC_POSITION)
	  (setq line (c-xref-server-dispatch-get-int-attr PPCA_LINE))
	  ...
	  (goto-char rpos)
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  (c-xref-server-dispatch-require-end-ctag PPC_LC_POSITION)
	  )
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_GOTO)
	i
))
...
(defun c-xref-server-dispatch-update-report (ss i len dispatch-data)
  (let ((tlen) (cc))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(while (and (< i len) (not (equal c-xref-server-ctag (format "/%s" PPC_UPDATE_REPORT))))
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	  (if (equal c-xref-server-ctag PPC_FATAL_ERROR)
		  (c-xref-server-dispatch-error ss i len dispatch-data PPC_FATAL_ERROR)
		)
	  (setq i (+ i tlen))
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  )
	(c-xref-server-dispatch-require-end-ctag PPC_UPDATE_REPORT)
	i
))
...
(defun c-xref-server-dispatch-replacement (ss i len dispatch-data)
  (let ((tlen) (str) (with) (slen) (rcc))
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_REFACTORING_REPLACEMENT)
...
(defun c-xref-server-dispatch-refactoring-cut-block (ss i len dispatch-data)
  (let ((blen))
	(setq blen (c-xref-server-dispatch-get-int-attr PPCA_VALUE))
	(setq c-xref-refactoring-block (buffer-substring (point) (+ (point) blen)))
	(delete-char blen t)
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_REFACTORING_CUT_BLOCK)
	i
))

(defun c-xref-server-dispatch-refactoring-paste-block (ss i len dispatch-data)
  (insert-string c-xref-refactoring-block)
  (setq i (c-xref-server-parse-xml-tag ss i len))
  (c-xref-server-dispatch-require-end-ctag PPC_REFACTORING_PASTE_BLOCK)
  i
)
...
(defun c-xref-server-dispatch-indent (ss i len dispatch-data)
  (let ((blen) (ii) (bb))
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_INDENT)
	i
))
...
(defun c-xref-server-dispatch-precheck (ss i len dispatch-data)
  (let ((tlen) (cc) (rcc))
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_REFACTORING_PRECHECK)
	i
))
...
(defun c-xref-server-dispatch-display-resolution (ss i len dispatch-data)
  (let ((tlen) (mess) (messagewin) (winconfig) (fdd))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_DISPLAY_RESOLUTION)
	(c-xref-select-dispach-data-caller-window dispatch-data)
	...
	(setq winconfig (current-window-configuration))
	  ...
	  (insert-string mess)
	  (insert-string c-xref-resolution-dialog-explication)
	  (goto-char (point-min))
...
(defun c-xref-server-dispatch-display-or-update-browser (ss i len dispatch-data)
  (let ((tlen) (mess) (messagewin))
	...
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_DISPLAY_OR_UPDATE_BROWSER)
	(c-xref-select-dispach-data-caller-window dispatch-data)
...
(defun c-xref-server-dispatch-classh-lines (ss i len dispatch-data ct-flag)
  (let ((tlen) (clas) (deps) (selected) (rd) (ru) (prefix) (sch) 
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(while (equal c-xref-server-ctag PPC_CLASS)
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	  (setq clas (c-xref-char-list-substring ss i (+ i tlen)))
	  (setq i (+ i tlen))
	  (setq deps (cdr (assoc PPCA_TREE_DEPS c-xref-server-ctag-attributes))) 
	  (setq selected (c-xref-server-dispatch-get-int-attr PPCA_SELECTED))
	  (setq rd (c-xref-server-dispatch-get-int-attr PPCA_DEF_REFN))
	  (setq ru (c-xref-server-dispatch-get-int-attr PPCA_REFN))
	  (setq defc (c-xref-server-dispatch-get-int-attr PPCA_DEFINITION))
	  (setq interfc (c-xref-server-dispatch-get-int-attr PPCA_INTERFACE))
	  (setq upp (c-xref-server-dispatch-get-int-attr PPCA_TREE_UP))
	  ...
	  (if (eq upp 1) (insert ")"))
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  (c-xref-server-dispatch-require-end-ctag PPC_CLASS)
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  )
	i
))
...
(defun c-xref-server-dispatch-symbol-resolution (ss i len dispatch-data)
  (let ((tlen) (symbol) (clas) (indent) (deps) (ru) (rd) 
	...
	(c-xref-erase-buffer)
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(while (or (equal c-xref-server-ctag PPC_VIRTUAL_SYMBOL)
			   (equal c-xref-server-ctag PPC_SYMBOL))
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	  (setq symbol (c-xref-char-list-substring ss i (+ i tlen)))
	  (setq i (+ i tlen))
	  (if (equal c-xref-server-ctag PPC_SYMBOL)
		  (progn
			(setq divnewline "\n")
			(setq selected (c-xref-server-dispatch-get-int-attr PPCA_SELECTED))
			(setq rd (c-xref-server-dispatch-get-int-attr PPCA_DEF_REFN))
			...
			(setq i (c-xref-server-parse-xml-tag ss i len))
			(c-xref-server-dispatch-require-end-ctag PPC_SYMBOL)
			(setq i (c-xref-server-parse-xml-tag ss i len))
			)
		(insert (format "%s\n== %s" divnewline symbol))
		(if (<= firstline 0) (setq firstline (+ (count-lines 1 (point)) 1)))
		(setq i (c-xref-server-parse-xml-tag ss i len))
		(c-xref-server-dispatch-require-end-ctag PPC_VIRTUAL_SYMBOL)
...
(defun c-xref-server-dispatch-reference-list (ss i len dispatch-data)
  (let ((tlen) (srcline) (n) (divnewline) (aline) (line) (j) (pointer)
	...
	(setq j 0)
	(setq divnewline "")
	(setq aline (c-xref-server-dispatch-get-int-attr PPCA_VALUE))
	(setq listed-symbol (cdr (assoc PPCA_SYMBOL c-xref-server-ctag-attributes)))
	...
	(c-xref-erase-buffer)
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(while (equal c-xref-server-ctag PPC_SRC_LINE)
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	  (setq srcline (c-xref-char-list-substring ss i (+ i tlen)))
	  (setq i (+ i tlen))
	  (setq n (c-xref-server-dispatch-get-int-attr PPCA_REFN))
	  (while (> n 0)
		(progn
		  (if (eq line aline) 
			  (setq pointer ">")
	  ...
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  (c-xref-server-dispatch-require-end-ctag PPC_SRC_LINE)
	  (if c-xref-xemacs-mule-problem (progn (setq len (- len i)) (setq ss (nthcdr i ss)) (setq j (+ j i)) (setq i 0)))
	  (setq i (c-xref-server-parse-xml-tag ss i len))
	  )
	(c-xref-server-dispatch-require-end-ctag PPC_REFERENCE_LIST)
...
;;(message "start %S len %d" (current-time) len)
	(while (equal c-xref-server-ctag PPC_STRING_VALUE)
	  (setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
...
;;(message "stop %S" (current-time))
	(c-xref-server-dispatch-require-end-ctag PPC_SYMBOL_LIST)
	(c-xref-hard-prepend-to-dispatch-data 
	 dispatch-data 
...
(defun c-xref-server-dispatch-move-file-as (ss i len dispatch-data)
  (let ((tlen) (cc) (rcc))
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_MOVE_FILE_AS)
	i
))
...
(defun c-xref-server-dispatch-extraction-dialog (ss i len dispatch-data)
  (let ((tlen) (minvocation) (mhead) (mtail) (mline) (dname))
	...
	(setq tlen (c-xref-server-dispatch-get-int-attr PPCA_LEN))
	(setq mline (c-xref-server-dispatch-get-int-attr PPCA_VALUE))
	(setq i (+ i tlen))
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_INT_VALUE)
	...
	(setq i (c-xref-server-parse-xml-tag ss i len))
	(c-xref-server-dispatch-require-end-ctag PPC_EXTRACTION_DIALOG)
	i
))
...
(defun c-xref-append-new-project-section (pname planguage pcomments 
  ...
  (let ((comment) (dlen) (cldopt) (tdq))
	...
	(if (or (equal planguage "j") (equal planguage "J")
			(equal planguage "b") (equal planguage "B"))
		(progn
		  (insert (format "  -set cp %s\n" (c-xref-optionify-string classpath "\"")))
		  (insert (format "  -set qcp \"${dq}${cp}${dq}\" \t// quoted classpath (if it contains spaces)\n"))
		  (if (not (equal javahome ""))
			  (progn
				(insert (format "  -set jhome \"%s\"\n" javahome))
				(insert (format "  -set jbin \"${jhome}%s\"\n" (c-xref-backslashify-name "bin/")))
				(insert (format "  -jdkclasspath \"${jhome}%s\"" 
									   (c-xref-backslashify-name "jre/lib/rt.jar")))
				(if comment
					(insert "\t// Java run time for c-xrefactory\n")
				  )
			))
		  (insert (format "  -classpath ${cp}"))
		  (if comment
			  (insert "\t// classpath for c-xrefactory\n")
			)
		  (insert (format "  -sourcepath %s" (c-xref-optionify-string sourcepath "\"")))
		  (if comment
			  (insert "\t// sourcepath for c-xrefactory\n")
			)
		  (if javadocpath
			  (progn
				(insert (format "  -javadocpath %s\n" (c-xref-optionify-string javadocpath "\"")))
				))
		  ...
		  (if classdir
			  (progn
				(setq cldopt (format "-d %s" (c-xref-optionify-string classdir "${dq}")))
				)
			(setq cldopt "")
			)
...
"
  ...
  (let ((pname) (checked) (tname) (breakcheck) (planguage) (pcomments)
	...
	(save-buffer) 
	(kill-buffer nil)
	(if (c-xref-buffer-has-one-of-suffixes (buffer-file-name) c-xref-java-suffixes)
		(setq ptdef "j")
	  (if (c-xref-buffer-has-one-of-suffixes (buffer-file-name) c-xref-c-suffixes)
		  (setq ptdef "c")
		(setq ptdef "b")
		))
	(setq planguage "c")
